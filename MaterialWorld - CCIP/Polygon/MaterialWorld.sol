...
contract MaterialWorld is  ERC721, ERC721Burnable, Ownable, CCIPReceiver, VRFV2WrapperConsumerBase  {
    ...
    function reroll_(uint tokenId) internal {
        ...
        uint8 rarity = findRarity(_randomWord, Trait.Boat); //returns 0-19

        uint newWealth = stats[tokenId].Wealth + wealthScores[Trait.Boat][rarity];
        if(stats[tokenId].Boat != 0) 
            newWealth -= wealthScores[Trait.Boat][stats[tokenId].Boat - 1];

        stats[tokenId].Boat = rarity + 1; //traits range from 1-20
        stats[tokenId].Wealth = newWealth;

        emit Material_log(stats[tokenId], tokenId, Trait.Boat, "Reroll Boat", msg.sender);
    }
    ...
    function rerollTraitVRF(uint tokenId) public payable {
        ...
        requests[requestId] = Request (true, "Reroll Boat", tokenId);
    }

    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        require(requests[_requestId].valid, "Invalid request id");

        uint8 rarity = findRarity(_randomWords[0], Trait.Boat); //returns 0-19

        Stat memory stat = stats[requests[_requestId].tokenId];
        
        uint newWealth = stat.Wealth + wealthScores[Trait.Boat][rarity];
        if(stat.Boat != 0) 
            newWealth -= wealthScores[Trait.Boat][stat.Boat - 1];

        stats[requests[_requestId].tokenId].Boat = rarity + 1; //traits range from 1-20
        stats[requests[_requestId].tokenId].Wealth = newWealth;
        requests[_requestId].valid = false;

        emit Material_log(stats[requests[_requestId].tokenId], requests[_requestId].tokenId, Trait.Boat, requests[_requestId].log, msg.sender);
    }
