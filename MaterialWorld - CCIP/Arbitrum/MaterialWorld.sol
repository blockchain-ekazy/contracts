...
contract MaterialWorld is  ERC721, ERC721Burnable, Ownable, CCIPReceiver, VRFV2WrapperConsumerBase  {
    ...
    function reroll_(uint tokenId) internal {
        ...
        uint8 rarity = findRarity(_randomWord, Trait.House); //returns 0-19

        uint newWealth = stats[tokenId].Wealth + wealthScores[Trait.House][rarity];
        if(stats[tokenId].House != 0) 
            newWealth -= wealthScores[Trait.House][stats[tokenId].House - 1];

        stats[tokenId].House = rarity + 1; //traits range from 1-20
        stats[tokenId].Wealth = newWealth;

        emit Material_log(stats[tokenId], tokenId, Trait.House, "Reroll House", msg.sender);
    }
    ...
    function rerollTraitVRF(uint tokenId) public payable {
        ...
        requests[requestId] = Request (true, "Reroll House", tokenId);
    }

    function fulfillRandomWords(uint256 _requestId, uint256[] memory _randomWords) internal override {
        require(requests[_requestId].valid, "Invalid request id");

        uint8 rarity = findRarity(_randomWords[0], Trait.House); //returns 0-19

        Stat memory stat = stats[requests[_requestId].tokenId];
        
        uint newWealth = stat.Wealth + wealthScores[Trait.House][rarity];
        if(stat.House != 0) 
            newWealth -= wealthScores[Trait.House][stat.House - 1];

        stats[requests[_requestId].tokenId].House = rarity + 1; //traits range from 1-20
        stats[requests[_requestId].tokenId].Wealth = newWealth;
        requests[_requestId].valid = false;

        emit Material_log(stats[requests[_requestId].tokenId], requests[_requestId].tokenId, Trait.House, requests[_requestId].log, msg.sender);
    }
