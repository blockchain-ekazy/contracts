...
contract MaterialWorld is  ERC721, ERC721Burnable, Ownable, CCIPReceiver  {
    ...
    constructor(address dinaraWallet, uint64 CCIP_ChainId_, address router, address aggregatorAddress) 
    Ownable(msg.sender) 
    CCIPReceiver(router) 
    ...
    function reroll_(uint tokenId) internal {
        ...
        uint8 rarity = findRarity(_randomWord, Trait.Character); //returns 0-19

        uint newWealth = stats[tokenId].Wealth + wealthScores[Trait.Character][rarity];
        if(stats[tokenId].Character != 0) 
            newWealth -= wealthScores[Trait.Character][stats[tokenId].Character - 1];

        stats[tokenId].Character = rarity + 1; //traits range from 1-20
        stats[tokenId].Wealth = newWealth;

        emit Material_log(stats[tokenId], tokenId, Trait.Character, "Reroll Character", msg.sender);
    }
    ...
    function findRarity(uint256 randomNumber, Trait trait) internal view returns (uint8) {
        ...
        // This should not happen under normal conditions, fallback return
        return 19;
    }
